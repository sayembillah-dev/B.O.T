...
//  sea at the bottom. Render: sky gradient + sun + clouds → parallax
//  mountain layers → dirt/grass body → water.
// ════════════════════════════════════════════════════════════════════
...
export async function generateTerrain(seed, width = 1920, height = 1080, onProgress) {
  ...
  const TOP = height * 0.14, BOT = height * 0.93; // BOT > waterY → sea inlets
  for (let x = 0; x < width; x++) {
...
export function destroyCircle(terrain, cx, cy, r) {
...
export function cleanDebris(terrain, cx, cy, r) {
  const { width, height, solid, surface } = terrain;
  const x0 = Math.max(0, Math.floor(cx - r)), x1 = Math.min(width - 1, Math.ceil(cx + r));
  for (let x = x0; x <= x1; x++) {
    for (let pass = 0; pass < 4; pass++) {
      ...
      if (b - a + 1 < 8 && airBelow) {
        for (let y = a; y <= b; y++) solid[y * width + x] = 0;
      } else break;
    }
    surface[x] = height;
...
export function removeFloaters(terrain, cx, cy, r) {
  const { width, height, solid, surface } = terrain;
  const x0 = Math.max(0, Math.floor(cx - r)), x1 = Math.min(width - 1, Math.ceil(cx + r));
  const y0 = Math.max(0, Math.floor(cy - r)), y1 = Math.min(height - 1, Math.ceil(cy + r));
  const bw = x1 - x0 + 1;
  ...
  for (let sy = y0; sy <= y1; sy++) {
    for (let sx = x0; sx <= x1; sx++) {
      ...
      if (seen[si] || !solid[sy * width + sx]) continue;
...
export function makeTerrainPainter(terrain) {
  const { width, height, surface, solid, openSky, waterY } = terrain;
  ...
  return function paint(x, y, out, o) {
    ...
    if (solid[i]) {
      ...
      const gt = grassT[x];
      if (depth < gt) {
        // grass skin: bright top highlight → mid green → dark root line
        const t = depth / gt;
        r = lerpN(124, 58, t); g = lerpN(208, 130, t * 0.85); b = lerpN(79, 40, t);
        if (depth < 1.6) { r = 150; g = 225; b = 95; }
      } else {
        // dirt: lighter topsoil → dark depths, mottled + faint strata
        const t = clamp01(depth / (height * 0.55));
        r = lerpN(126, 62, t); g = lerpN(84, 40, t); b = lerpN(50, 26, t);
    ...
    if (!openSky[i]) {
      // ─── CAVE INTERIOR ───
      const v = 26 + grainAt(x, y) * 5;
      out[o] = v; out[o + 1] = v * 0.72; out[o + 2] = v * 0.55; out[o + 3] = 255;
      return;
    }

    // ─── SKY ───
    const t = clamp01(y / horizon);
    r = lerpN(lerpN(38, 127, t), 217, smoothstep(0.75, 1, t));
    g = lerpN(lerpN(70, 178, t), 232, smoothstep(0.75, 1, t));
    b = lerpN(lerpN(115, 229, t), 245, smoothstep(0.75, 1, t));
    ...
    // clouds (stretched stratus puffs)
    if (y < height * 0.5) {
      const c = fbm(cloudN, x / width * 3.2, y / height * 7.5, 3);
      const ca = smoothstep(0.06, 0.4, c) * (1 - smoothstep(0.32, 0.5, y / height)) * 0.85;
      r = lerpN(r, 255, ca); g = lerpN(g, 252, ca); b = lerpN(b, 246, ca);
    }

    // parallax mountain silhouettes (atmospheric fade)
    if (y > farY[x]) { const a = 0.62; r = lerpN(r, 147, a); g = lerpN(g, 169, a); b = lerpN(b, 196, a); }
    if (y > nearY[x]) { const a = 0.95; r = lerpN(r, 93, a); g = lerpN(g, 116, a); b = lerpN(b, 142, a); }

    // ─── WATER (open air below waterline) ───
    if (y > waterY) {
      const wob = Math.sin(x * 0.035) * 2.5 + fbm(nearN, x / width * 20, 5.5, 2) * 2;
      const d = clamp01((y - waterY) / (height - waterY));
      const wr = lerpN(46, 16, d), wg = lerpN(111, 52, d), wb = lerpN(158, 84, d);
      const a = 0.88;
      r = lerpN(r, wr, a); g = lerpN(g, wg, a); b = lerpN(b, wb, a);
      if (y - waterY < 3 + wob) { r = 168; g = 216; b = 240; } // sparkling surface line
    }
...
export function renderTerrainRGBA(terrain) {
  ...
  const paint = makeTerrainPainter(terrain);
