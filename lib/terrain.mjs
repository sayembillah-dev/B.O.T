...
export function makeTerrainPainter(terrain) {
  ...
  return function paint(x, y, out, o) {
    ...
    if (y > waterY) {
      ...
      const wr = lerpN(38, 12, d), wg = lerpN(126, 56, d), wb = lerpN(170, 94, d);
      const a = 0.9;
      r = lerpN(r, wr, a); g = lerpN(g, wg, a); b = lerpN(b, wb, a);
      // soft underwater light bands drifting with depth
      const band = Math.sin(y * 0.09 + fbm(nearN, x / width * 8, y / height * 8, 2) * 5) * 7 * (1 - d);
      r += band * 0.5; g += band; b += band;
      if (y - waterY < 3 + wob) { r = 178; g = 228; b = 246; } // sparkling surface line
      else if (y - waterY < 8 + wob * 1.6) { r += 26; g += 30; b += 28; } // sub-surface glow
    }
