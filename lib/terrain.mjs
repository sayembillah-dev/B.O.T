...
export async function generateTerrain(seed, width = 1920, height = 1080, onProgress) {
  ...
  const terrain = { width, height, seed: String(seed), surface, solid, openSky, waterY, stamps: stampsApplied, theme: themeFor(seed).id };
  onProgress?.(1);
  return terrain;
}
...
export function makeTerrainPainter(terrain) {
  ...
  return function paint(x, y, out, o) {
    ...
    if (solid[i]) {
      ...
      const xa = Math.max(0, x - 2), xb = Math.min(width - 1, x + 2);
      ...
      if (depth < gt) { // grass fringe: lit tips, 1px highlight, root shadow
        const t = depth / gt;
        const clump = fbm(grassN, x / width * 46, 2.2, 2);
        const dap = (grainAt(x, y) - 0.5) * 22;
        if (depth < 1.8) {
        ...
        if (fbm(dirtN, x / width * 72, y / height * 72, 2) > 0.34) {
      ...
      if (depth >= grassT[x] && airAbove(x, y)) { r *= th.rim[0]; g *= th.rim[1]; b *= th.rim[2]; }
      out[o] = r; out[o + 1] = g; out[o + 2] = b; out[o + 3] = 255;
      return;
    }

    if (!openSky[i]) { // cave interior, theme-tinted
      const v = th.cave.base + grainAt(x, y) * 9 + fbm(dirtN, x / width * 12, y / height * 12, 2) * 10;
    ...
    if (y > farY[x]) { // parallax silhouettes
      const a = 0.62;
      r = lerpN(r, th.hills.far[0], a); g = lerpN(g, th.hills.far[1], a); b = lerpN(b, th.hills.far[2], a);
      if (y - farY[x] < 7 && farY[x] < height * 0.42) { const s = 0.5; r = lerpN(r, th.hills.farCap[0], s); g = lerpN(g, th.hills.farCap[1], s); b = lerpN(b, th.hills.farCap[2], s); }
    }
    if (y > nearY[x]) { const a = 0.95; r = lerpN(r, th.hills.near[0], a); g = lerpN(g, th.hills.near[1], a); b = lerpN(b, th.hills.near[2], a); }
