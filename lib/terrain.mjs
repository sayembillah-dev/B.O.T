...
export async function generateTerrain(seed, width = 1920, height = 1080, onProgress) {
  ...
  // ── A7: seeded props (cosmetic vector silhouettes - rocks/bones/trees/
  //    dead trunks/crystals, weighted per theme). Surface-anchored, spawn
  //    pads avoided, max ~14 per map. Render-only: Game.jsx bakes them onto
  //    the terrain canvas; a prop is burned away the moment its host pixel
  //    is destroyed (isSolid check on dirty-rect re-render). The list is
  //    derived from the shared seed, so every client rebuilds it identically.
  const pRng = mulberry32(hashSeed(`${seed}:props`));
  const PROP_KINDS = { // weighted kind tables per theme
    highland: ['tree', 'tree', 'tree', 'rock', 'rock', 'trunk'],
    desert: ['bones', 'bones', 'rock', 'rock', 'trunk', 'trunk'],
    tundra: ['trunk', 'trunk', 'crystal', 'crystal', 'rock', 'rock'],
    volcanic: ['crystal', 'crystal', 'crystal', 'rock', 'rock', 'trunk'],
  };
  const PROP_SIZE = { rock: [8, 15, 7, 13], tree: [10, 16, 30, 52], bones: [12, 20, 8, 14], trunk: [6, 10, 20, 34], crystal: [8, 14, 12, 26] }; // hw lo/hi, ht lo/hi
  const kindTable = PROP_KINDS[themeFor(seed).id] ?? PROP_KINDS.highland;
  const maxProps = Math.min(14, Math.max(6, Math.round(width / 192))); // ~10 @2P … 14 @5P+
  const props = [];
  for (let attempt = 0; attempt < 60 && props.length < maxProps; attempt++) {
    const a = pRng(), b = pRng(), c = pRng(), d = pRng(), e = pRng(), f = pRng(); // fixed draw order
    const kind = kindTable[(b * kindTable.length) | 0];
    const sz = PROP_SIZE[kind];
    const hw = Math.round(sz[0] + c * (sz[1] - sz[0]));
    const ht = Math.round(sz[2] + d * (sz[3] - sz[2]));
    const x = Math.round(84 + hw + a * (width - 168 - 2 * hw));
    let bad = false;
    for (const p of padYs) { // spawn pads stay clean
      if (Math.abs(x - p.px) < PAD * 2 + hw + 20) { bad = true; break; }
    }
    if (!bad) for (const q of props) { // breathing room between props
      if (Math.abs(x - q.x) < hw + q.hw + 44) { bad = true; break; }
    }
    if (!bad) { // no props straddling cliffs / steep slopes
      const sL = surface[Math.max(0, x - hw)], sC = surface[x], sR = surface[Math.min(width - 1, x + hw)];
      if (Math.max(sL, sC, sR) - Math.min(sL, sC, sR) > 18) bad = true;
    }
    if (bad) continue;
    const y = surface[x];
    props.push({ k: kind, x, y, hw, ht, f: e < 0.5, v: f, ay: Math.ceil(y) }); // ay = host pixel row
  }

  const terrain = { width, height, seed: String(seed), surface, solid, openSky, waterY, stamps: stampsApplied, theme: themeFor(seed).id, scorch: new Uint8Array(width * height), props }; // A6 crater-scorch layer (render-only, rebuilt from the replayed blast log)
  onProgress?.(1);
  return terrain;
}
...
// == A7 seeded props ==========================================================
// Cosmetic vector silhouettes baked onto the terrain canvas. Zero physics /
// protocol impact: the list comes from the shared seed (generateTerrain), so
// every client draws the same props; a prop vanishes the moment its host
// pixel is destroyed - checked against the live bitmap at every repaint.

function shadeRGB(c, k) { // k<0 darken toward 0, k>0 lighten toward 255
  const t = k < 0 ? 1 + k : 1, u = k < 0 ? 0 : k;
  return [
    Math.round(c[0] * t + (255 - c[0]) * u),
    Math.round(c[1] * t + (255 - c[1]) * u),
    Math.round(c[2] * t + (255 - c[2]) * u),
  ];
}
...
const cssRGB = (c, a) => (a === undefined || a >= 1 ? `rgb(${c[0]},${c[1]},${c[2]})` : `rgba(${c[0]},${c[1]},${c[2]},${a})`);

/** Physics-space bounds of a prop silhouette (base embedded ~3px, +1px margin). */
export function propBounds(p) {
  return { x: p.x - p.hw - 1, y: p.y - p.ht - 1, w: p.hw * 2 + 2, h: p.ht + 4 };
}

/** Burned props: host pixel gone AND bounds intersect this physics-space
 *  rect. Game.jsx erases these boxes with fresh terrain paint, then redraws
 *  the survivors - a prop never leaves stale pixels in the sky. */
export function propsGone(terrain, region) {
  const { solid, width, props } = terrain;
  if (!props || !props.length) return [];
  const x1 = region.x + region.w, y1 = region.y + region.h;
  const gone = [];
  for (const p of props) {
    if (solid[p.ay * width + p.x]) continue; // host alive
    const b = propBounds(p);
    if (b.x >= x1 || b.x + b.w <= region.x || b.y >= y1 || b.y + b.h <= region.y) continue;
    gone.push(b);
  }
  return gone;
}

/** Draw living props onto a baked terrain canvas. ss = bake supersample
 *  factor; clip = device-space rect ({x,y,w,h}) for dirty-rect repaints
 *  (null = full bake). Host-solidity is re-checked per draw, so survivors
 *  only. */
export function drawProps(ctx, terrain, ss = 1, clip = null) {
  const props = terrain.props;
  if (!ctx || !props || !props.length) return;
  const { solid, width } = terrain;
  const th = themeFor(terrain.seed);
  const crysBase = mixRGB(th.hills.farCap, th.sun.disc, 0.25);
  const crysHiC = mixRGB(th.sun.disc, [255, 255, 255], 0.3);
  const pal = {
    rock: cssRGB(shadeRGB(th.dirt.pebble, -0.38)),
    rockHi: cssRGB(shadeRGB(th.dirt.pebble, 0.22), 0.85),
    leaf: cssRGB(shadeRGB(th.grass.top, -0.35)),
    leafHi: cssRGB(shadeRGB(th.grass.tip, 0.05), 0.5),
    wood: cssRGB(shadeRGB(th.dirt.deep, -0.2)),
    bone: cssRGB(mixRGB(th.hills.farCap, [255, 255, 255], 0.45)),
    boneSh: cssRGB(shadeRGB(th.hills.farCap, -0.35), 0.6),
    crys: cssRGB(crysBase),
    crysLo: cssRGB(shadeRGB(crysBase, -0.25)),
    crysHi: cssRGB(crysHiC, 0.9),
    crysIn: cssRGB(crysHiC, 0.35),
  };
  let cx0 = -Infinity, cy0 = -Infinity, cx1 = Infinity, cy1 = Infinity;
  ctx.save();
  if (clip) {
    ctx.beginPath(); ctx.rect(clip.x, clip.y, clip.w, clip.h); ctx.clip();
    cx0 = clip.x / ss; cy0 = clip.y / ss; cx1 = (clip.x + clip.w) / ss; cy1 = (clip.y + clip.h) / ss;
  }
  if (ss !== 1) ctx.scale(ss, ss);
  for (const p of props) {
    if (!solid[p.ay * width + p.x]) continue; // burned away with its host
    if (p.x + p.hw < cx0 || p.x - p.hw > cx1 || p.y + 3 < cy0 || p.y - p.ht > cy1) continue;
    drawProp(ctx, p, pal);
  }
  ctx.restore();
}

function drawProp(ctx, p, pal) {
  const x = p.x, y = p.y, hw = p.hw, ht = p.ht, v = p.v, sgn = p.f ? 1 : -1;
  if (p.k === 'rock') { // lumpy dome, sun edge from the top-left (A4 lighting)
    const px = x - hw * 0.4 + v * hw * 0.25, qh = ht * (0.72 + 0.28 * v);
    ctx.fillStyle = pal.rock;
    ctx.beginPath();
    ctx.moveTo(x - hw, y + 2);
    ctx.lineTo(x - hw * 0.92, y - ht * 0.5);
    ctx.lineTo(px, y - ht);
    ctx.lineTo(x + hw * 0.5, y - qh);
    ctx.lineTo(x + hw * 0.95, y - ht * 0.3);
    ctx.lineTo(x + hw, y + 2);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle = pal.rockHi; ctx.lineWidth = 1.2;
    ctx.beginPath(); ctx.moveTo(x - hw * 0.9, y - ht * 0.48); ctx.lineTo(px, y - ht); ctx.lineTo(x + hw * 0.5, y - qh); ctx.stroke();
  } else if (p.k === 'tree') { // pine silhouette: trunk + 3 stacked tiers
    const lean = (v - 0.5) * 4 * sgn;
    ctx.fillStyle = pal.wood;
    ctx.fillRect(x - 1.6, y - ht * 0.3, 3.2, ht * 0.3 + 2);
    ctx.fillStyle = pal.leaf;
    const TW = [hw, hw * 0.75, hw * 0.48], TB = [0.16, 0.40, 0.64], TA = [0.46, 0.70, 1.0];
    for (let i = 0; i < 3; i++) {
      const by = y - ht * TB[i], ay = y - ht * TA[i], ax = x + lean * (0.4 + 0.3 * i);
      ctx.beginPath();
      ctx.moveTo(x - TW[i], by); ctx.lineTo(ax, ay); ctx.lineTo(x + TW[i], by);
      ctx.closePath(); ctx.fill();
    }
    ctx.strokeStyle = pal.leafHi; ctx.lineWidth = 1; // sun-side tier edges
    ctx.beginPath();
    for (let i = 0; i < 3; i++) {
      const by = y - ht * TB[i], ay = y - ht * TA[i], ax = x + lean * (0.4 + 0.3 * i);
      ctx.moveTo(x - TW[i], by); ctx.lineTo(ax, ay);
    }
    ctx.stroke();
  } else if (p.k === 'bones') { // half-buried ribcage + skull
    ctx.strokeStyle = pal.boneSh; ctx.lineWidth = 2.4; // ground shadow under the spine
    ctx.beginPath(); ctx.moveTo(x - hw, y - 0.5); ctx.quadraticCurveTo(x, y - ht * 0.4 + 1.5, x + hw, y - 2.5); ctx.stroke();
    ctx.strokeStyle = pal.bone; ctx.lineWidth = 2.2;
    ctx.beginPath(); ctx.moveTo(x - hw, y - 2); ctx.quadraticCurveTo(x, y - ht * 0.4, x + hw, y - 4); ctx.stroke();
    ctx.lineWidth = 1.6;
    ctx.beginPath();
    for (let i = 0; i < 4; i++) {
      const bx = x - hw * 0.55 + i * hw * 0.36;
      ctx.moveTo(bx, y - 3.5);
      ctx.quadraticCurveTo(bx + hw * 0.26, y - ht * 0.55, bx + hw * 0.1, y - ht * (0.8 + 0.12 * ((i + ((v * 3) | 0)) % 2)));
    }
    ctx.stroke();
    ctx.fillStyle = pal.bone;
    ctx.beginPath(); ctx.arc(x - hw * 0.85, y - ht * 0.45, ht * 0.4, 0, 6.2832); ctx.fill();
  } else if (p.k === 'trunk') { // jagged dead trunk, two stub branches
    const lean = (v - 0.5) * 6 * sgn;
    ctx.fillStyle = pal.wood;
    ctx.beginPath();
    ctx.moveTo(x - hw * 0.55, y + 2);
    ctx.lineTo(x + hw * 0.55, y + 2);
    ctx.lineTo(x + hw * 0.18 + lean * 0.5, y - ht * 0.5);
    ctx.lineTo(x + lean, y - ht);
    ctx.lineTo(x - hw * 0.2 + lean * 0.55, y - ht * 0.45);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle = pal.wood; ctx.lineWidth = 2;
    ctx.beginPath(); ctx.moveTo(x + lean * 0.5, y - ht * 0.55); ctx.lineTo(x + sgn * (hw + 5), y - ht * 0.8); ctx.stroke();
    ctx.lineWidth = 1.5;
    ctx.beginPath(); ctx.moveTo(x + lean * 0.35, y - ht * 0.38); ctx.lineTo(x - sgn * (hw + 2), y - ht * 0.58); ctx.stroke();
  } else { // crystal: main shard + smaller side shard, lit top edges
    const lean = (v - 0.5) * 3;
    const sx = x - sgn * hw * 0.6, sh = ht * 0.55, sw = hw * 0.5;
    ctx.fillStyle = pal.crysLo;
    ctx.beginPath();
    ctx.moveTo(sx - sw * 0.5, y + 1.5);
    ctx.lineTo(sx - sw * 0.28, y - sh * 0.7);
    ctx.lineTo(sx, y - sh);
    ctx.lineTo(sx + sw * 0.32, y - sh * 0.6);
    ctx.lineTo(sx + sw * 0.5, y + 1.5);
    ctx.closePath(); ctx.fill();
    ctx.fillStyle = pal.crys;
    ctx.beginPath();
    ctx.moveTo(x - hw * 0.5, y + 1.5);
    ctx.lineTo(x - hw * 0.3, y - ht * 0.7);
    ctx.lineTo(x + lean, y - ht);
    ctx.lineTo(x + hw * 0.32, y - ht * 0.62);
    ctx.lineTo(x + hw * 0.5, y + 1.5);
    ctx.closePath(); ctx.fill();
    ctx.strokeStyle = pal.crysHi; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(x - hw * 0.3, y - ht * 0.7); ctx.lineTo(x + lean, y - ht); ctx.lineTo(x + hw * 0.32, y - ht * 0.62); ctx.stroke();
    ctx.strokeStyle = pal.crysIn;
    ctx.beginPath(); ctx.moveTo(x + lean, y - ht); ctx.lineTo(x - hw * 0.05, y + 1); ctx.stroke();
  }
}
