...
/** Player display name (localStorage so it survives a full browser restart;
 *  legacy sessionStorage copy migrated once). Empty string when unknown. */
export function getPlayerName() {
  if (typeof window === 'undefined') return '';
  const name = localStorage.getItem('player-name') || sessionStorage.getItem('player-name') || '';
  if (name) { try { localStorage.setItem('player-name', name); } catch { /* storage blocked */ } }
  return name;
}

/** Persist the display name (best-effort - storage may be blocked). */
export function savePlayerName(name) {
  if (typeof window === 'undefined') return;
  try { localStorage.setItem('player-name', name); } catch { /* storage blocked */ }
}
